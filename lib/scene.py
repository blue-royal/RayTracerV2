...
class Scene:
    def __init__(self):
        self.objects = [Sphere(GREEN, Vec3(0.1, 0.5, 3), 1.5), Sphere(WHITE, Vec3(-2, -1.5, 2.5), 0.5)] #, Sphere(MELON, Vec3(1, -0.5, 3), 0.6), Sphere(DEEP_BLUE, Vec3(-0.8, 00, 8), 1), Sphere(DEEP_BLUE, Vec3(0.9, 0.9, 1), 0.2)]
        ...
        for object in self.objects:
            if object.collision_dist(ray) < min_dist:
                ...
                if min_dist != INFINITY:
                    pixel_colour = object.material.get_colour(self.lights, ray(min_dist), self, ray)
        if min_dist == INFINITY:
